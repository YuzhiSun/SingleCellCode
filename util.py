...
def concat_express_and_label_and_gcn_data(file_name = 'mouse10_100'):
    source_gene_express = pd.read_csv('../data/SingleCellData/mouse/mouse2_merge.csv', index_col=0)
    target_gene_express = pd.read_csv('../data/SingleCellData/mouse/mouse1_merge.csv', index_col=0)
    gene_gcn = pd.read_csv('../data/net/gene_gcn_feas.csv')
    src_label = pd.read_csv('../data/SingleCellData/mouse/mouse2_cell_result.csv')
    tar_label = pd.read_csv('../data/SingleCellData/mouse/mouse1_cell_result.csv')
...
def make_train_valid():
    version = '0711_19'
    data = pd.read_csv(fr'../data/train/gene_express_label_gcn.csv')
    x_data = data.iloc[:,:-1]
    y_data = data.iloc[:,-1]
    train_x, test_x, train_y, test_y = train_test_split(x_data, y_data, test_size=0.2, random_state=32, stratify=y_data)
    train_data = pd.concat([train_x,train_y],axis=1)
    valid_data = pd.concat([test_x,test_y],axis=1)
    a = Counter(train_y)  # 训练集里面各个标签的出现次数
    b = Counter(test_y)  # 测试集里面各个标签的出现次数
    c = Counter(y_data)
    train_data.to_csv(fr'../data/train/train_data{version}.csv',index=False)
    valid_data.to_csv(fr'../data/train/valid_data{version}.csv',index=False)
    print()
